	...
	go func() {
		for {
			...
			case <-ui.renderTicker.C:
				// Rendering now happens via Render() method
			case <-ui.stopChan:
...
// Render returns the complete UI as a string.
func (ui *ScannerUI) Render() string {
	ui.mu.Lock()
	defer ui.mu.Unlock()
	...
	output.WriteString(ui.renderRepositories())

	// Render scanner status
	output.WriteString(ui.renderScanners())

	// Render findings summary
	output.WriteString(ui.renderSummary())

	// Render errors if any
	if len(ui.errorMessages) > 0 {
		output.WriteString(ui.renderErrors())
	}

	return output.String()
}

// RenderFinalState returns the final UI state with the given summary.
func (ui *ScannerUI) RenderFinalState(summaryLines []string) string {
	ui.mu.Lock()
	defer ui.mu.Unlock()
	...
	if len(summaryLines) > 0 {
		output.WriteString(ui.drawBox("✨ Scan Complete!", summaryLines))
	}

	return output.String()
}

func (ui *ScannerUI) renderHeader() string {
	...
	return ui.drawBox("Prismatic Security Scanner", lines)
}

func (ui *ScannerUI) renderRepositories() string {
	...
	return ui.drawBox("Repository Preparation", lines)
}

func (ui *ScannerUI) renderScanners() string {
	...
	return ui.drawBox("Scanner Status", table)
}
...
func (ui *ScannerUI) renderSummary() string {
	total := 0
